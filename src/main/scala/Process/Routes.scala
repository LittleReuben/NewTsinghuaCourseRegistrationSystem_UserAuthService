...
import Impl.VerifyTokenValidityMessagePlanner
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "UserLogoutMessage" =>
        IO(
          decode[UserLogoutMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for UserLogoutMessage[${err.getMessage}]")
      ...
      case "VerifyTokenValidityMessage" =>
        IO(
          decode[VerifyTokenValidityMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for VerifyTokenValidityMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
